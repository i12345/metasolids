...
export function field_point_stdDev<Point extends FieldPoint = FieldPoint>(points: Point[]): number {
    if (points[0] instanceof Vec3)
        return fields_point_stdDev(points as FieldsPoint[])
    else if (points[0] instanceof Mat4)
        return fields_point_stdDev(points.map(point => trs(point as Mat4)))
        ...
        return fields_point_stdDev(points.map(point => (point as Quat).getEulerAngles() as unknown as FieldsPoint))
    else if (points[0] instanceof Mat3)
        return fields_point_stdDev(points.map(point => trs(mat4_from_mat3(point as Mat3))))
        ...
        return stdDevs_sum / points[0].length
    }
    else return fields_point_stdDev(points as FieldsPoint[])
}

export function fields_point_stdDev<Point extends FieldsPoint = FieldsPoint>(points: Point[]): number {
    let squares = 0

    for (const key of Reflect.ownKeys(points[0])) {
        const extracted = points.map(point => point[key])
        const stdDev = field_point_stdDev(extracted)
        squares += stdDev ** 2
    }
